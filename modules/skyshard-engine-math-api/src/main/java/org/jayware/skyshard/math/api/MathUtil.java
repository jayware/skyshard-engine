...
{
    public static final double PIOVER180 = Math.PI / 180.0;
    public static final double EPS = 0.000001;

	public static void setPerspectiveProjectionMatrix(Matrix4f matrix,
			float width, float height, float nearPlane, float farPlane, float fieldOfView)
	{
		matrix.setIdentity();

		float aspectRatio = width / height;
		float y_scale = (float) (1f / Math.tan(Math.toRadians(0.5 * fieldOfView)));
		float x_scale = y_scale / aspectRatio;
		float frustumLength = farPlane - nearPlane;

		matrix.setElement(0, 0, x_scale);
		matrix.setElement(1, 1, y_scale);
		matrix.setElement(2, 2, -((farPlane + nearPlane) / frustumLength));
		matrix.setElement(2, 3, -1);
		matrix.setElement(3, 2, -((2 * nearPlane * farPlane) / frustumLength));
		matrix.setElement(3, 3, 0);
	}

	public static void setOrthographicProjectionMatrix(Matrix4f matrix,
			float width, float height, float nearPlane, float farPlane)
	{
		...
		matrix.setElement(0, (float) 2.0f / (r - l));
		matrix.setElement(5, (float) 2.0f / (t - b));
		matrix.setElement(10, (float) -2.0f / (farPlane - nearPlane));
		matrix.setElement(3, -((r + l) / (r - l)));
		matrix.setElement(7, -((t + b) / (t - b)));
		matrix.setElement(11, -((farPlane + nearPlane) / (farPlane - nearPlane)));
	}
	...
	public static void mat_mul(double[] m1, double[] m2, double[] m3)
	{
		int i;
	...
	public static void transpose_mat(double[] in, double[] out)
	{
		out[0] = in[0];
		out[1] = in[3];
		out[2] = in[6];
	...
	public static double compute_shift(double f, double g, double h)
	{
		double d__1, d__2;
	...
	public static int compute_qr(double[] s, double[] e, double[] u, double[] v)
	{
...
//		double[] m = new double[9]; // TODO: Remove if not needed.
...
//			m[8] = s[2];
	...
	public static double compute_rot(double f, double g, double[] sin,
			double[] cos, int index, int first)
	{
		int i__1;
		double d__1, d__2;
		double cs, sn;
	...
	public static void compute_svd(double[] m, double[] outScale,
			double[] outRot)
	{
		int i, j;
		double g, scale;
	...
	public static void svdReorder(double[] m, double[] t1, double[] t2,
    ...
    public static double inDegree(double radians)
    {
        return radians  * (180.0 / Math.PI);
    }

    public static float inDegree(float radians)
    {
    ...
    public static float inRadians(float degree)
    {
